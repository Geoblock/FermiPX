...
//---------------------------------------------------------------------------
// Block
VoxelCube*** InitBlock(DelaunayBase* Points, int sizeCube, int sizeBlock)
{
	int size = (sizeCube / sizeBlock) * 2;
	int i, j, t, x, y, z;
	VoxelCube*** blocks;
	blocks = new VoxelCube**[size];
	for (int i = 0; i < size; ++i) {
		blocks[i] = new VoxelCube*[size];
		for (int j = 0; j < size; ++j)
			blocks[i][j] = new VoxelCube[size];
	}
	for (int k = 0; k < 7; k++) {
		for (i = 0; i < Points[k].nodeCount; i++) {
			x = (sizeCube + Points[k].node[i][0]) / sizeBlock;
			y = (sizeCube + Points[k].node[i][1]) / sizeBlock;
			z = (sizeCube + Points[k].node[i][2]) / sizeBlock;
			blocks[x][y][z].Increase(k);
		}
	}
	for (i = 0; i < size; i++) {
		for (j = 0; j < size; j++) {
			for (t = 0; t < size; t++) {
				blocks[i][j][t].TakeMostClass();
			}
		}
	}
...
int*** Block(DelaunayBase dt_struct, int sizeCube, int sizeBlock)
{
	int size = (sizeCube / sizeBlock) * 2;
	int i, j, t, x, y, z;
	int*** dataVoxel;
    ...
    for (i = 0; i < dt_struct.nodeCount; i++) {
        x = (sizeCube + dt_struct.node[i][0]) / sizeBlock;
        y = (sizeCube + dt_struct.node[i][1]) / sizeBlock;
		z = (sizeCube + dt_struct.node[i][2]) / sizeBlock;
        dataVoxel[x][y][z] += 1;
    }
...
void __fastcall TFormSpace::DrawBlocks()
{
    ...
    switch (ClassGroup->ItemIndex) {
        case 0:
			blockData = Block(All_Delaunay, size, cubeSize);
			R = All_Delaunay.color[0];
			G = All_Delaunay.color[1];
			B = All_Delaunay.color[2];
			break;
		case 1:
			blockData = Block(O_Delaunay, size, cubeSize);
			R = O_Delaunay.color[0];
			G = O_Delaunay.color[1];
			B = O_Delaunay.color[2];
			break;
		case 2:
			blockData = Block(B_Delaunay, size, cubeSize);
			R = B_Delaunay.color[0];
			G = B_Delaunay.color[1];
			B = B_Delaunay.color[2];
			break;
		case 3:
			blockData = Block(A_Delaunay, size, cubeSize);
			R = A_Delaunay.color[0];
			G = A_Delaunay.color[1];
			B = A_Delaunay.color[2];
			break;
		case 4:
			blockData = Block(F_Delaunay, size, cubeSize);
			R = F_Delaunay.color[0];
			G = F_Delaunay.color[1];
			B = F_Delaunay.color[2];
			break;
		case 5:
			blockData = Block(G_Delaunay, size, cubeSize);
			R = G_Delaunay.color[0];
			G = G_Delaunay.color[1];
			B = G_Delaunay.color[2];
			break;
		case 6:
			blockData = Block(K_Delaunay, size, cubeSize);
			R = K_Delaunay.color[0];
			G = K_Delaunay.color[1];
            B = K_Delaunay.color[2];
            break;
        case 7:
			blockData = Block(M_Delaunay, size, cubeSize);
			R = M_Delaunay.color[0];
			G = M_Delaunay.color[1];
			B = M_Delaunay.color[2];
			break;
...
void __fastcall TFormSpace::Start1Click(TObject* Sender)
{
    ...
    String filenames[14];
	filenames[0] = "O_Delaunay.sqlite";
    filenames[1] = "O_Voronoi.sqlite";
    filenames[2] = "A_Delaunay.sqlite";
    filenames[3] = "A_Voronoi.sqlite";
    filenames[4] = "B_Delaunay.sqlite";
    filenames[5] = "B_Voronoi.sqlite";
	filenames[6] = "F_Delaunay.sqlite";
    filenames[7] = "F_Voronoi.sqlite";
